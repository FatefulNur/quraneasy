  ...
  "submodules": [
    {
      ...
      // slice of an ayah recitation) | false (disabled, e.g. waqf signs).
      // Omitted = speech-synthesis fallback. See src/lib/audio.ts.
...
- **Audio**: all recitation is **AbdulBaset Murattal** from audio.qurancdn.com (reciter id 2). Ayah examples play full-verse recitation parsed from the `reference` field (`RECITER_BASE` in `src/lib/audio.ts`) — **never TTS for Quranic text** (wrong tajweed). Word/letter examples play real recitation via the optional `audio` field (word-by-word clips, or timed AbdulBaset segments); junction rules (idgham/ikhfa/iqlab across words) must use continuous segments, pausal forms (madd arid/leen, qalqalah kubra) must use ayah-final segments-to-end. Word-by-word clips (`audio.qurancdn.com/wbw/`) are a separate per-word reciter, used for single isolated words. Web Speech API (`ar-SA`) is the fallback for unannotated (non-Quranic) words only. Segment audio is generated/regenerated by `scripts/apply-word-audio.py` from its decision table. All logic in `src/lib/audio.ts` + `src/lib/useAudio.ts`; MP3s cached client-side via Cache Storage (`qe-audio-v1`, 24h TTL).
